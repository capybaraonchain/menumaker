...
test('ingredient remediation searches source-backed nutrition foods through shared actions and MCP', () => {
  ...
  assert.match(webPage, /aria-label="Fuente nutricional"/)
  assert.match(webPage, /source: nextSourceFilter \|\| undefined/)
...
test('queued generation jobs have a local worker entrypoint', () => {
  ...
  assert.match(appActions, /processQueuedGenerationJobs: z\.object/)
  assert.match(appActions, /runQueuedGenerationJobs\(input\.limit, input\.profileId\)/)
  assert.match(mcp, /'process_generation_queue'/)
  assert.match(apiRoute, /processQueuedGenerationJobs: 'processQueuedGenerationJobs'/)
  assert.match(webPage, /Generar semana \(en segundo plano\)/)
  assert.match(webPage, /runNow: false/)
  assert.match(webPage, /Generar semana ahora/)
