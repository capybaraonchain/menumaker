...
| Failure UI | Generation failures are visible from persisted jobs with Spanish explanations, retry action, and code-specific remediation metadata | Partial live implementation | `apps/web/app/page.tsx`, `packages/db/src/appService.ts`, `packages/db/src/remediation.ts`, `apps/mcp/src/server.ts` | Failed/running jobs appear on Semana/Historial, failed jobs include specific next steps, and unresolved repair issues surface targeted remediation | Remediation is inline rather than a full guided remediation workflow with editable ingredient/preference forms | Add guided remediation flows for ingredient mapping, preference relaxation, target edits, and repair-specific regeneration |
