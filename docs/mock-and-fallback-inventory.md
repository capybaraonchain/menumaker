...
| Generation Jobs | Weekly generation and long preview flows now use queued job rows plus explicit runners; app state, UI, and MCP expose job status and retry | Partial live implementation | `packages/db/src/appService.ts`, `packages/db/src/appActions.ts`, `apps/web/app/page.tsx`, `apps/mcp/src/server.ts`, `generation_jobs` table | Weekly generation input and preview input are serialized into the job, then a runner moves it queued -> running -> completed/failed; users and agents can inspect real logs/failure codes | Local v1 still runs jobs inside request/agent calls; no background worker, streaming, cancellation, or worker process yet | Move job execution to a worker/queue, stream progress, support cancellation, and add UI progress controls for preview jobs |
...
| MCP Regeneration Previews | Regeneration and calorie previews return server-owned plans, and MCP can also queue/run preview jobs that persist the exact plan in `generation_jobs.result.plan` | Live for synchronous previews and preview-job runners; broader worker coverage still partial | `apps/mcp/src/server.ts`, `packages/db/src/appService.ts`, `packages/db/src/appActions.ts` | External agents can inspect or persist the exact plan before confirmation | Preview jobs still run on demand inside the request; no background worker, streaming progress, or cancellation | Move preview execution to async workers with progress logs, cancellation, and UI controls |
...
5. Move job execution and queued preview runners to background async workers.
