...
| Nutrition Data | Deterministic scoring can now read imported source records plus user-confirmed aliases; seed foods remain the local baseline | Partially live source-backed | `packages/nutrition/src/engine.ts`, `packages/db/src/appService.ts`, `packages/db/src/nutritionSourceImport.ts`, `packages/db/src/seed.ts` | Lets BEDCA/USDA/Open Food Facts-style records become the scoring source without changing recipe/menu code | Coverage still depends on importing datasets; no built-in remote dataset downloader or source picker exists yet | Add official source adapters/downloaders, dataset version metadata, source selection UI, and custom user foods |
| Nutrition Source Tables | Source schemas support idempotent imported records and are read by generation, edit, and calorie-adjustment scoring | Live import path, dataset adapters still pending | `packages/db/src/migrate.ts`, `packages/db/src/schema.ts`, `packages/db/src/seed.ts`, `packages/db/src/nutritionSourceImport.ts`, `packages/db/src/appService.ts` | Keeps the future data model in use rather than only mirrored from seed rows | Imported files must already be normalized into the app import shape | Add BEDCA/USDA/Open Food Facts import adapters and record dataset versions |
...
1. Add official BEDCA/USDA/Open Food Facts adapters on top of the source-record import path.
