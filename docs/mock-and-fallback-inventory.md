...
| Nutrition Data | Deterministic scoring can now read imported source records plus user-confirmed aliases; seed foods remain the local baseline | Partially live source-backed | `packages/nutrition/src/engine.ts`, `packages/db/src/appService.ts`, `packages/db/src/nutritionSourceImport.ts`, `packages/db/src/openFoodFactsImport.ts`, `packages/db/src/seed.ts` | Lets Open Food Facts barcode products and normalized BEDCA/USDA-style records become the scoring source without changing recipe/menu code | Coverage still depends on importing datasets or barcodes; no source picker or custom user foods exists yet | Add USDA/BEDCA adapters, dataset version metadata, source selection UI, and custom user foods |
| Nutrition Source Tables | Source schemas support idempotent imported records and are read by generation, edit, and calorie-adjustment scoring | Live import path plus Open Food Facts barcode adapter | `packages/db/src/migrate.ts`, `packages/db/src/schema.ts`, `packages/db/src/seed.ts`, `packages/db/src/nutritionSourceImport.ts`, `packages/db/src/openFoodFactsImport.ts`, `packages/db/src/appService.ts` | Keeps the future data model in use rather than only mirrored from seed rows | Open Food Facts covers packaged products; generic-food source adapters are still pending | Add BEDCA/USDA adapters and record dataset versions |
