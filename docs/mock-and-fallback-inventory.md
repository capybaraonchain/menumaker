...
| AI Cache | Successful structured recipe, week-skeleton, chat-planner, and menu-chat calls are cached | Partial live implementation | `packages/db/src/appService.ts`, `packages/db/src/migrate.ts`, `packages/db/src/schema.ts`, `apps/web/app/api/chat/route.ts` | Reduces repeated recipe/skeleton/chat calls and records generated outputs | Repair requests and generation summaries still lack cache/trace records | Extend cache helper to repair requests, generation summaries, and code-specific remediation planning |
...
2. Extend AI cache and trace records to repair requests, generation summaries, and remediation planning.
