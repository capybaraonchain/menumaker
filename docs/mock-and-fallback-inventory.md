...
| Nutrition Data | Seed nutrition dataset instead of full source coverage | Active fallback/substitute | `packages/nutrition/src/seedFoods.ts`, `packages/db/src/seed.ts`, `packages/nutrition/src/engine.ts` | Enables deterministic calories/macros before BEDCA/USDA/Open Food Facts import exists | LLM must stay inside a small ingredient universe; valid recipes with unknown ingredients are rejected or underpowered | Add source adapters, import/version source records, richer aliases, serving/household unit data, and confidence UI |
| Nutrition Source Tables | Source schemas exist but are populated only from seed data | Schema-ready but incomplete | `packages/db/src/migrate.ts`, `packages/db/src/schema.ts`, `packages/db/src/seed.ts` | Keeps the future data model in place | "database" confidence currently means local seed-backed, not full authoritative coverage | Populate `source_foods`, `nutrition_records`, and `food_mappings` from real datasets and record dataset versions |
| Unit Conversion | Generic household unit approximations | Rough deterministic fallback | `packages/nutrition/src/engine.ts` | Lets recipes with `cucharada`, `unidad`, or `rebanada` calculate something | A "unidad" is not the same across eggs, bananas, bread, etc. | Add per-food unit conversion records and require LLM recipes to prefer grams/ml when possible |
| Recipe Generation | Deterministic recipe templates remain as fallback | Acceptable fallback, not primary | `packages/nutrition/src/recipes.ts`, `packages/db/src/appService.ts`, `packages/db/src/caloriePlanner.ts` | Keeps app usable if Codex OAuth is missing, stale, slow, or returns too few valid candidates | Could hide provider failures or make output feel repetitive if overused | Keep recording `recipeSource`; surface fallback state in logs/UI; reduce dependence as nutrition coverage and LLM retry/caching improve |
...
| AI Cache | `ai_cache` table exists but generation calls do not use it yet | Schema-only placeholder | `packages/db/src/migrate.ts`, `packages/db/src/schema.ts` | Planned in ADRs before full cache integration | Repeated similar requests call the model again; harder to audit generated candidates | Add cache helper keyed by model, schema version, prompt hash, profile/menu hash, and candidate kind |
| MCP Regeneration Previews | Regeneration previews are lightweight descriptions | Partial proposal tools | `apps/mcp/src/server.ts` | Avoided expensive candidate generation in early MCP slice | External agents cannot inspect exact candidate outcomes before confirming regeneration | Make preview tools produce server-owned plans with candidate summaries and menu hash, then apply exact preview |
...
| Data Reset/Admin | Profile wipe/reset is manual/dev-only | Operational shortcut | Local DB commands, no admin UI | Personal local app | Easy to test locally, but no app-native data management | Add profile deletion/reset tools with confirmation, audit event, and optional export |
...
2. Add AI cache and generation trace records.
3. Replace lightweight MCP regeneration previews with server-owned preview plans.
4. Add full week skeleton generation and repair-loop orchestration.
5. Add richer generation failure UI.
