...
| Recipe Candidate Scoring | Week selection uses deterministic scoring and bounded repair, not a full optimizer | Pragmatic v1 heuristic | `packages/db/src/appService.ts`, `packages/db/src/weekPlanning.test.ts` | Avoids obvious repeats, daily calorie drift, and weak weekly protein before persistence | May miss better global menu combinations | Add a proper menu-level optimizer with candidate pools, constraints, beam search/local search, and richer scoring traces |
| Week Skeleton | Weekly generation now creates a first-class `WeekSkeleton` before recipe candidates | LLM-first with explicit local fallback | `packages/ai/src/planning.ts`, `packages/db/src/appService.ts`, ADR 0005 | Variety planning is passed into recipe generation and stored in generation settings | Deterministic fallback still exists for missing/failed provider and no user-facing skeleton editor exists | Add richer skeleton review/editing, async progress, and source-specific failure UI |
| Repair Loop | Weekly assembly has bounded deterministic repair for repetition, daily calorie drift, and low weekly protein | Partial live implementation | `packages/db/src/appService.ts`, `packages/db/src/caloriePlanner.ts`, `packages/db/src/weekPlanning.test.ts` | Repairs obvious quality failures using the same validated candidate pools before persistence | Not yet a full LLM repair controller with structured `RepairRequest` / `RepairResult` jobs | Add repair jobs, retry logs, LLM repair proposals, and failure explanations when deterministic repair cannot fix a menu |
| Generation Jobs | Jobs persist and are now exposed through app state, UI, and MCP with retry for failed jobs | Partial live implementation | `packages/db/src/appService.ts`, `apps/web/app/page.tsx`, `apps/mcp/src/server.ts`, `generation_jobs` table | Users and agents can inspect real job logs/failure codes instead of generic errors | Generation execution is still synchronous request-local work; no worker queue, streaming, or cancellation yet | Move generation to worker/queue, stream progress, support cancellation, and run previews as jobs |
| AI Cache | Successful structured recipe and week-skeleton generations are cached; chat/repair calls are not yet cached | Partial live implementation | `packages/db/src/appService.ts`, `packages/db/src/migrate.ts`, `packages/db/src/schema.ts` | Reduces repeated recipe/skeleton calls and records generated outputs | Chat planning and repair requests still lack cache/trace records | Extend cache helper to chat planning, repair requests, and generation summaries |
...
| Failure UI | Generation failures are visible from persisted jobs with Spanish explanations and retry action | Partial live implementation | `apps/web/app/page.tsx`, `packages/db/src/appService.ts`, `apps/mcp/src/server.ts` | Failed/running jobs appear on Semana/Historial and can be retried when safe | Failure handling is still job-level, not a full dedicated remediation screen per failure code | Add code-specific remediation screens for low nutrition confidence, ambiguous ingredient, banned conflict, repetition conflict, and generation exhausted |
...
| Evaluation | Basic weekly quality scenarios now exist | Partial quality harness | `packages/db/src/weekPlanning.test.ts`, `packages/db/src/caloriePlanner.test.ts` | Repetition and daily-calorie drift regressions are covered by automated tests | Deliciousness, edit-flow variety, banned-food propagation, and real LLM output quality still need fixtures | Add golden scenario tests and scored fixtures for meal edit, banned foods, LLM candidates, and variety |
...
2. Extend AI cache and trace records beyond recipe/skeleton generation.
3. Expand repair-loop orchestration from bounded deterministic repair to structured async repair jobs.
4. Add code-specific generation failure remediation UI.
