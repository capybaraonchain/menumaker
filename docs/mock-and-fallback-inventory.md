...
| --- | --- | --- | --- | --- | --- | --- |
| Auth | Single local user stub with enforced owned-table `user_id` coverage | Intentional v1 local stub with hosted-readiness guard | `packages/db/src/env.ts`, `packages/db/src/appService.ts`, `packages/db/src/appActions.ts`, `packages/db/src/schemaOwnership.test.ts` | Local personal runtime on the MacBook Air M2 | No real multi-user session isolation; RLS/auth not exercised at runtime | Add hosted auth, real user identity, session mapping, and RLS policy tests while preserving existing `user_id` columns |
| Sync | Hosted sync deferred, but generated outputs including AI cache are user-scoped | Intentional v1 deferral with schema guard | ADR 0003, ADR 0007, checklist, `packages/db/src/schemaOwnership.test.ts` | Personal local app first | Mobile/desktop state only works against the local server/database | Supabase/Postgres hosted deployment, account system, migrations, RLS, profile ownership validation |
| Nutrition Data | Deterministic scoring can now read imported source records, user-defined foods, USDA downloadable dataset foods, and user-confirmed aliases; seed foods remain the local baseline | Partially live source-backed | `packages/nutrition/src/engine.ts`, `packages/db/src/appService.ts`, `packages/db/src/appActions.ts`, `packages/db/src/nutritionSourceImport.ts`, `packages/db/src/openFoodFactsImport.ts`, `packages/db/src/usdaFdcDownloadImport.ts`, `packages/db/src/seed.ts`, `apps/mcp/src/server.ts` | Lets Open Food Facts barcode products, USDA FoodData Central downloadable JSON records, normalized BEDCA-style records, and user-entered per-100g foods become the scoring source without changing recipe/menu code | Coverage still depends on importing datasets, barcodes, or user-entered facts; no dedicated source picker UI exists yet | Add BEDCA import adapter, dataset version metadata UI, and richer source-selection UI |
| Nutrition Source Tables | Source schemas support idempotent imported records, user-defined foods, USDA downloaded files, and app/MCP mutations; generation, edit, and calorie-adjustment scoring read them | Live import path plus Open Food Facts, USDA download, and custom-food actions | `packages/db/src/migrate.ts`, `packages/db/src/schema.ts`, `packages/db/src/seed.ts`, `packages/db/src/nutritionSourceImport.ts`, `packages/db/src/openFoodFactsImport.ts`, `packages/db/src/usdaFdcDownloadImport.ts`, `packages/db/src/appActions.ts`, `apps/mcp/src/server.ts`, `packages/db/src/appService.ts` | Keeps the future data model in use rather than only mirrored from seed rows | Open Food Facts covers packaged products; USDA download covers generic international foods after local import; Spanish BEDCA adapter is still pending; custom foods depend on user-entered nutrition facts | Add BEDCA adapter and record dataset versions |
...
| Generation Jobs | Weekly generation and long preview flows now use queued job rows plus explicit runners; app state, UI, and MCP expose job status, retry, and cancellation | Partial live implementation | `packages/db/src/appService.ts`, `packages/db/src/appActions.ts`, `apps/web/app/page.tsx`, `apps/mcp/src/server.ts`, `generation_jobs` table | Weekly generation input and preview input are serialized into the job, then a runner moves it queued -> running -> completed/failed/cancelled; users and agents can inspect real logs/failure codes and stop queued/running jobs | Local v1 still runs jobs inside request/agent calls; cancellation is checkpoint-based for in-flight work, with no separate background worker or streaming | Move job execution to a worker/queue, stream progress, and add stronger worker-level cancellation |
...
| Nutrition Confidence UX | Low-confidence/unknown ingredient failures can open guided ingredient mapping; agents can import OFF barcodes or create user-defined foods before mapping aliases | Partial live implementation | `packages/nutrition/src/engine.ts`, `packages/db/src/appService.ts`, `packages/db/src/appActions.ts`, `apps/web/app/page.tsx`, `apps/mcp/src/server.ts` | v1 rejects bad candidates before UI, but the user can save aliases, add packaged products, or add per-100g custom foods and retry through the same deterministic scorer | The in-app UI still has a simple mapping form; source creation is primarily action/MCP-driven | Add a fuller in-app source picker/creation form and confidence explanations |
| Failure UI | Generation failures are visible from persisted jobs with Spanish explanations, retry action, code-specific remediation metadata, guided preference relaxation, guided ingredient mapping, and targeted repair-regeneration actions | Partial live implementation | `apps/web/app/page.tsx`, `packages/db/src/appService.ts`, `packages/db/src/appActions.ts`, `packages/db/src/remediation.ts`, `apps/mcp/src/server.ts` | Failed/running jobs appear on Semana/Historial, failed jobs include specific next steps, preference constraints can be relaxed, unknown aliases can be mapped, and unresolved repair notices can regenerate the affected meal/day/week through typed actions | Target editing and fallback-policy remediation forms are still not guided forms, though fallback can be toggled from Perfil or remediation actions | Add guided remediation flows for target edits and fallback policy changes |
...
2. Expand deterministic unit conversions and in-app source-selection UX.
...
5. Add guided remediation flows for failed generation and unresolved repair cases.
6. Move job execution and queued preview runners from request-time runners to background async workers.
