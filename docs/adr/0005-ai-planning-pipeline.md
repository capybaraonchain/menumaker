...
Local v1 now includes a bounded deterministic repair pass during weekly assembly. It checks obvious repetition conflicts, absurd daily calorie drift, banned-food leakage, unknown nutrition, and low weekly protein before the menu is persisted. Each attempted repair emits structured `RepairRequest` / `RepairResult` telemetry into the weekly menu and generation job trace. If unresolved repair issues remain, the app turns the persisted affected day/slot context into targeted regenerate-meal, regenerate-day, or regenerate-week actions. This is not the final full LLM repair controller; future repair jobs should run asynchronously, allow LLM-authored proposals, and preserve retry logs.
...
- Cached generation-summary trace when the local LLM provider is configured.
